...
mkv_df = pd.read_csv(os.path.join(os.path.dirname(os.path.realpath(__file__)),'mkvalue.csv'))
mkv_df = mkv_df[['code', 'mktcap']]
mkv_dict={}
for index in range(len(mkv_df)):
    mkv_dict[int(mkv_df.iloc[index].code)]=mkv_df.iloc[index].mktcap

mkv_sorted = sorted(mkv_dict.items(), key=lambda d: d[1], reverse=True)
mkv_sorted = mkv_sorted[:500]
# print(mkv_sorted)
# exit()

with open('vote_500_industry.csv','w',encoding='utf8') as f:
    head = 'code, 行业, 占比, 行业, 占比, 行业, 占比, | , 产品, 占比, 产品, 占比, 产品, 占比, -'
    f.write(head+'\n')
    for it in mkv_sorted:
        co = it[0]
        code= '0000000'+str(co)
        code = code[-6:]
